...
short RtemsMidiDriver::AddInputDevice (int device_num, MidiInputDriver* pDriver)
{
  //## begin RtemsMidiDriver::AddInputDevice%1012952111.body preserve=yes
	...
	cout<<"RtemsMidiDriver::AddInputDevice"<<device_num<<endl;
#endif
...
#ifdef	SHOW_DIAG
							cout<<"Request num rx data. Received "<<(unsigned int)(io_values[dev_num].num_rx_data_bytes)<<endl;
#endif 
...
#ifdef  SHOW_DIAG
							cout<<"Retransmit "<<io_out_data<<endl;
#endif 
...
#ifdef	SHOW_DIAG
							cout<<"Request num tx data. Transmitted "<<(unsigned int)(io_values[dev_num].num_tx_data_bytes)<<endl;
#endif 
...
#ifdef	SHOW_DIAG
							cout<<"Unknown Diag Message. Re-transmit "<<io_out_data<<endl;
#endif 
...
bool RtemsMidiDriver::WriteOutputReady (int device_num)
{
  //## begin RtemsMidiDriver::WriteOutputReady%1013139105.body preserve=yes
	...
	if (TX_DATA_MASK & io_in_data) // then we can proceed. Otherwise get out quickly
		{
			if (pMidiOut[device_num]->GetTxReady())
				{
